...
contract Fee1155 is ERC1155, Ownable {
  ...
  /**
    ...
    @param _feeOwner The address of a FeeOwner who receives earnings from this
                     item.
  */
  constructor(string memory _uri, FeeOwner _feeOwner) public ERC1155(_uri) {
    metadataUri = _uri;
    feeOwner = _feeOwner;
    nextItemGroupId = 0;
  }
  ...
  */
  function approveMinter(address _minter, bool _approval) public onlyOwner {
  ...
  */
  function create(uint256[] calldata initialSupply, uint256[] calldata _maximumSupply, address[] calldata recipients, bytes calldata data) public onlyOwner returns (uint256) {
    ...
    for (uint256 i = 0; i < groupSize; i++) {
      ...
      fullCollectionSize.add(itemMaximumSupply);
  ...
  */
  function mint(address to, uint256 id, uint256 amount, bytes calldata data) public onlyMinters {
    uint256 shiftedGroupId = id << 128;
    currentSupply[shiftedGroupId].add(amount);
  ...
  /**
    ...
    @param data Any associated data for this minting event that should be passed.
  */
  function mintBatch(address to, uint256[] calldata ids, uint256[] calldata amounts, bytes calldata data) public onlyMinters {
