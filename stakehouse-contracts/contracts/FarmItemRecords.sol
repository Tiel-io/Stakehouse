...
/**
  @title A basic smart contract for tracking the ownership of Stakehouse Items.
  @author Tim Clancy

  This is the governing registry of all Stakehouse Item assets.
*/
contract FarmItemRecords is Ownable {
  ...
  */
  function createItem(string calldata _uri, uint256 _royaltyFee, uint256[] calldata _initialSupply, uint256[] calldata _maximumSupply, address[] calldata _recipients, bytes calldata _data) external returns (Fee1155) {
    FeeOwner royaltyFeeOwner = new FeeOwner(_royaltyFee, 30000);
    Fee1155 newItemGroup = new Fee1155(_uri, royaltyFeeOwner);
