...
*/
contract Shop1155 is Ownable, ERC1155Holder {
  ...
  */
  function addStakers(Staker[] memory _stakers) public onlyOwner {
  ...
  */
  function listItems(PricePair[] memory _pricePairs, Fee1155[] calldata _items, uint256[][] calldata _ids, uint256[][] calldata _amounts) public onlyOwner {
    ...
    for (uint256 i = 0; i < _items.length; i++) {
      ...
      for (uint256 j = 0; j < ids.length; j++) {
        ...
        for (uint k = 0; k < _pricePairs.length; k++) {
          prices[nextItemId][k] = _pricePairs[k];
        }
  ...
  */
  function removeItem(uint256 _itemId, uint256 _amount) public onlyOwner {
  ...
  */
  function changeItemPrice(uint256 _itemId, PricePair[] memory _pricePairs) public onlyOwner {
  ...
  */
  function purchaseItem(uint256 _itemId, uint256 _amount, uint256 _assetId) public payable {
    ShopItem storage item = inventory[_itemId];
    require(item.amount >= _amount && item.amount != 0,
    ...
    // If the sentinel value for the Ether asset type is found, sell for Ether.
      ...
      payable(item.token.feeOwner().owner()).transfer(royaltyValue);
      payable(owner()).transfer(msg.value.sub(feeValue).sub(royaltyValue));
      inventory[_itemId].amount = inventory[_itemId].amount.sub(_amount);
      item.token.safeTransferFrom(address(this), msg.sender, item.id, _amount, "");
