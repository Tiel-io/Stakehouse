...
describe('FarmRecords', function () {
	let alice, bob, dev;
	let FeeOwner, FarmTokenRecords, FarmStakerRecords, FarmShopRecords, FarmItemRecords, Token, Staker, Shop1155, Fee1155;
	before(async () => {
	...
	let platformFeeOwner, farmTokenRecords, farmStakerRecords, farmShopRecords, farmItemRecords;
	beforeEach(async () => {
		...
		farmItemRecords = await FarmItemRecords.connect(dev.signer).deploy();
	...
	it('should allow users to create Stakers', async () => {
		...
		let stakerCreatedTransaction = await farmStakerRecords.connect(alice.signer)
			.createFarm('Staker', newTokenAddress, 100, [ 0 ], [ ethers.utils.parseEther('1') ]);
