...
describe('Shop1155', function () {
	...
	let Token, Staker, FeeOwner, Shop1155, Fee1155;
	before(async () => {
	...
	let token, staker, platformFeeOwner, shop, itemFeeOwner, itemOne;
	beforeEach(async () => {
		...
		itemOne = await Fee1155.connect(alice.signer).deploy('', itemFeeOwner.address);
		await itemOne.deployed();
		await itemOne.connect(alice.signer).create([ 10 ], [ 10 ], [ alice.address ], []);
		...
		await staker.connect(alice.signer).addEmissions([ 0 ],
	...
	it('should be able to sell items to users', async () => {
		...
		(finalBobPoints - bobPoints).should.be.equal(300);
	});
	...
	it('should allow the shop owner to change item prices', async () => {
		...
		shopBalance.should.be.equal(8);
		let finalBobPoints = await staker.connect(bob.signer).getAvailablePoints(bob.address);
		(finalBobPoints - bobPoints).should.be.equal(400);
	});
