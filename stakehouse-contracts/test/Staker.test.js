...
describe('Staker', function () {
	...
	it('can only add a pool after defining the emission schedule', async () => {
		await expect(
			staker.connect(alice.signer).addPool(token.address, 100)
		).to.be.revertedWith('Staking pools cannot be addded until an emission schedule has been defined.');
		await staker.connect(alice.signer).addEmissions([ 0, 10, 20 ], [ 10, 5, 1 ]);
		await staker.connect(alice.signer).addPool(token.address, 100);
	});
	...
	describe('-> After initialization of emission schedule ...', function () {
		beforeEach(async () => {
			...
			await staker.connect(alice.signer).addEmissions([ 1000, 1010, 1020 ],
				[ ethers.utils.parseEther('10'), ethers.utils.parseEther('5'), ethers.utils.parseEther('1') ]);
			await staker.connect(alice.signer).addPool(token.address, 100);
		});
		...
		it('should properly reward users for their stakes', async () => {
			...
			let bobPendingTokens = await staker.connect(bob.signer).pendingTokens(token.address, bob.address);
			bobPendingTokens.should.be.equal(ethers.utils.parseEther('77.5'));
			let carolPendingTokens = await staker.connect(carol.signer).pendingTokens(token.address, carol.address);
			...
			carolPoints.should.be.equal(7900);
			carolTotalPoints = await staker.connect(carol.signer).getTotalPoints(carol.address);
			carolTotalPoints.should.be.equal(7900);
			carolSpentPoints = await staker.connect(carol.signer).getSpentPoints(carol.address);
			carolSpentPoints.should.be.equal(0);
			bobPendingTokens = await staker.connect(bob.signer).pendingTokens(token.address, bob.address);
			bobPendingTokens.should.be.equal(ethers.utils.parseEther('0'));
			carolPendingTokens = await staker.connect(carol.signer).pendingTokens(token.address, carol.address);
